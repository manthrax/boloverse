...
function loadMap(ascmap) {
    ...
    for (t = 0; t < imap.length; t++)
        imap[t] =
            (hexStr.indexOf(ascmap[mi++]) << 4) | hexStr.indexOf(ascmap[mi++]); //.charCodeAt(t)&255;

    var tag = "BMAPBOLO";
    var tagValid = true;
    for (t = 0; t < 8; t++)
        if (tag.charAt(t).charCodeAt() != imap[t]) tagValid = false;
    if (tagValid == false) {
    ...
    var appearsValid =
        ...
        imap[t++] == 255 &&
        imap[t++] == 255;
    if (appearsValid == false) console.log("AppearsValid:" + appearsValid);
