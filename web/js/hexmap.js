...
define(["util/domReady!", // Waits for page load
    ...
    */
    ...
    function buildSphere(){
        ...
        let tuvi=[  0,7,6,1,8,7,2,9,8,3,10,9,4,11,10,5,6,11,
            6,7,12,7,8,13,8,9,14,9,10,15,10,11,16,11,0,17,
            0,7,6,1,8,7,2,9,8,3,10,9,4,11,10,5,6,11];

        let verts=[];
        let norms=[];
        for(let vii=0;vii<vis.length;vii++){verts=verts.concat(vts[vis[vii]]);norms=norms.concat(vts[vis[vii]]);}
        let uvs=[];
        for(vii=0;vii<tuvi.length;vii++)uvs=verts.concat(tuv[tuvi[vii]]);
        let inds=[];
        for(let t=0;t<vis.length;t++){inds=inds.concat([t]);}
